...
const NEXT_STATUS: Record<string, { label: string; status: string; color: string }> = {
  ...
  LIVE: { label: 'End Competition', status: 'COMPLETED', color: 'text-muted border-line-strong hover:text-fg' },
};
    ...
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center">
        <div className="text-center text-muted">
          ...
          <button
            onClick={() => signIn()}
            className="px-4 py-2 rounded-xl bg-accent/10 border border-accent/30 text-accent text-sm font-bold hover:bg-accent/20 transition-all"
          >
            Sign In
          </button>
        </div>
    ...
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center">
        <div className="text-center text-muted">
          ...
          <Link href="/" className="mt-3 inline-block text-accent text-sm underline">Go home</Link>
        </div>
      </div>
    );
  }
      ...
      <div className="bg-surface border-b border-line px-4 sm:px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-black text-fg">Admin Panel</h1>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-accent/10 border border-accent/30 text-accent text-sm font-bold hover:bg-accent/20 transition-all"
            >
          ...
          {[
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                activeTab === id ? 'bg-line text-fg' : 'text-muted hover:text-fg'
              }`}
            >
              {label}
              {count != null && (
                <span className="text-[10px] bg-line-strong px-1.5 py-0.5 rounded-full">{count}</span>
              )}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'competitions' && (
          <div className="space-y-3">
            ...
            {!loading && competitions.length === 0 && (
              <div className="text-center py-10 bg-surface border border-line rounded-2xl">
                <Database size={32} className="mx-auto mb-3 text-muted opacity-40" />
                <p className="text-sm text-muted mb-2">No competitions yet.</p>
                <p className="text-xs text-muted">Use Dev Tools to seed sample data, or create one above.</p>
              </div>
            )}
            {competitions.map(comp => {
              ...
              return (
                <div
                  key={comp._id}
                  className="bg-surface border border-line rounded-2xl p-4 hover:border-line-strong transition-all"
                >
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <h3 className="font-semibold text-sm text-fg">{comp.name}</h3>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Link
                        ...
                        target="_blank"
                        className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs text-muted border border-line-strong hover:text-fg transition-all"
                      >
            ...
            <div className="space-y-4 max-w-lg">
              <div className="bg-surface border border-line rounded-2xl p-5">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-semibold text-sm text-fg">Seed Sample Data</h3>
                </div>
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-accent/10 border border-accent/30 text-accent text-sm font-bold hover:bg-accent/20 transition-all disabled:opacity-50"
                >
        ...
        <div className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2">
              <label className="text-xs text-muted mb-1 block">Competition Name *</label>
              <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                className="w-full px-3 py-2 bg-elevated border border-line-strong rounded-xl text-sm text-fg focus:outline-none focus:border-accent"
          ...
          <div>
            <label className="text-xs text-muted mb-2 block">Events</label>
          ...
          <div className="flex items-center gap-6">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm text-fg">Free competition</span>
            </label>
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.isPractice}
                onChange={e => setForm(f => ({ ...f, isPractice: e.target.checked }))} />
              <span className="text-sm text-fg">Practice event (unlisted — direct link only)</span>
            </label>
          </div>
          {!form.isFree && (
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-muted mb-1 block">Base Fee (₹)</label>
                <input type="number" min="0" step="0.01" value={form.baseFee}
                  onChange={e => setForm(f => ({ ...f, baseFee: e.target.value }))}
                  className="w-full px-3 py-2 bg-elevated border border-line-strong rounded-xl text-sm text-fg focus:outline-none focus:border-accent" />
              </div>
              <div>
                <label className="text-xs text-muted mb-1 block">Per Event Fee (₹)</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="text-xs text-muted mb-1 block">Max Entries</label>
              <input type="number" value={form.maxEntries}
                onChange={e => setForm(f => ({ ...f, maxEntries: e.target.value }))}
                className="w-full px-3 py-2 bg-elevated border border-line-strong rounded-xl text-sm text-fg focus:outline-none focus:border-accent" />
            </div>
            <div>
              <label className="text-xs text-muted mb-1 block">Rounds</label>
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs text-muted">🏆 Prize Distribution</label>
            ...
            <div className="space-y-2">
              {form.prizes.map((p, i) => (
                <div key={i} className="bg-elevated border border-line-strong rounded-xl p-2.5 space-y-2">
                  <div className="flex items-center gap-2">
                    {/* Mode toggle */}
                    <div className="flex rounded-lg overflow-hidden border border-line-strong text-[11px] shrink-0">
                      {(['fixed', 'pool'] as const).map(m => (
                        <button key={m} type="button"
                          onClick={() => updatePrizeTier(i, { mode: m })}
                          className={`px-2 py-1 transition-all ${
                            p.mode === m ? 'bg-accent/20 text-accent' : 'text-muted hover:text-fg'
                          }`}>
                          {m === 'fixed' ? 'Fixed' : 'Pool'}
                        </button>
                      ))}
                    ...
                    <div className="flex items-center gap-1 text-[11px] text-muted">
                      ...
                      <input type="number" min="1" value={p.rankStart}
                        ...
                        className="w-12 px-1.5 py-1 bg-bg border border-line-strong rounded text-center text-fg focus:outline-none focus:border-accent" />
                      <span>–</span>
                    ...
                    <button type="button" onClick={() => removePrizeTier(i)}
                      className="ml-auto p-1 text-muted hover:text-red-400 transition-all" aria-label="Remove tier">
                      <Trash2 size={13} />
                    </button>
                  </div>
                  <div className="flex items-center gap-2">
                    {p.mode === 'fixed' ? (
                      <div className="flex items-center gap-1.5 flex-1">
                        <span className="text-[11px] text-muted">₹ each</span>
                        <input type="number" min="0" step="0.01" value={p.amount}
                          ...
                          className="flex-1 px-2 py-1 bg-bg border border-line-strong rounded-lg text-sm text-fg focus:outline-none focus:border-accent" />
                      </div>
                    ) : (
                      <>
                        <div className="flex items-center gap-1.5 flex-1">
                        ...
                        <select value={p.distribution}
                          ...
                          className="px-2 py-1 bg-bg border border-line-strong rounded-lg text-[11px] text-fg focus:outline-none focus:border-accent">
                          {DISTRIBUTIONS.map(d => (
            ...
            <button type="button" onClick={addPrizeTier}
              className="mt-2 flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs text-accent border border-accent/30 hover:bg-accent/10 transition-all">
          ...
          <div>
            ...
            <textarea value={form.description}
              ...
              rows={3}
              className="w-full px-3 py-2 bg-elevated border border-line-strong rounded-xl text-sm text-fg focus:outline-none focus:border-accent resize-none"
              placeholder="Competition description..." />
          </div>
          <div className="flex justify-end gap-2 pt-2 border-t border-line">
            <button onClick={() => setShowCreate(false)}
              className="px-4 py-2 rounded-xl text-sm text-muted hover:text-fg hover:bg-line transition-all">
              Cancel
            </button>
