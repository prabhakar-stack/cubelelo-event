...
export default function CompetitionResults({ params }: PageParams) {
  ...
  // Live leaderboard via SSE — pushes updates without polling.
  useEffect(() => {
    if (!competition) return;
    const compId = competition.competitionId ?? id;
    const es = new EventSource(`/api/competitions/${compId}/leaderboard/stream?eventId=${encodeURIComponent(selectedEvent)}`);
    es.addEventListener('leaderboard', (e: MessageEvent) => {
      try { const d = JSON.parse(e.data); setResults(d.leaderboard ?? []); } catch { /* ignore */ }
    });
  ...
  if (!competition) {
    return (
      <div className="min-h-screen bg-bg flex items-center justify-center text-muted">
        ...
        <Link href="/competitions" className="text-accent underline ml-1">Back</Link>
      </div>
    );
  }
  ...
  async function downloadCertificate() {
    ...
    try {
      ...
      a.href = url;
      a.download = `certificate-${compId}-${selectedEvent}.pdf`;
      a.click();
